...
int main(int argc, char **argv) {
    ...
    Matrix3d R = AngleAxisd(M_1_PI/2, Vector3d(0,0,1)).toRotationMatrix();
    // or quaternion
    Quaterniond q(R);
    ...
    cout << "The matrix form of the SO(3) made from R & q..." << endl;
    cout << "SO(3) from matrix:\n" << SO3_R.matrix() << "\n" << endl;
    cout << "SO(3) from quaternion:\n" << SO3_q.matrix() << "\n\n" << endl;
    cout << "they are equal" << endl;

    // Use logarithmic map to get the Lie algebra
    Vector3d so3 = SO3_R.log(); //which is a 3D vector
    cout << "so3 = " << so3.transpose() << "\n" << endl;
    // hat(^) is from vector to skew−symmetric matrix
    cout << "so3 hat=\n" << Sophus::SO3d::hat(so3) << "\n" << endl;
    // vee(v) is inversely from matrix to vector
    cout << "so3 hat vee= " << Sophus::SO3d::vee(Sophus::SO3d::hat(so3)).transpose() << "\n" << endl;

    // update by perturbation model
    Vector3d update_so3(1e-4, 0, 0); // this is a small update / deltachange
    Sophus::SO3d SO3_updated = Sophus::SO3d::exp(update_so3) * SO3_R;
    cout << "SO3 updated (this is the derivative wrt so(3) lie algebra) = \n" << SO3_updated.matrix() << "\n" << endl;
    ...
    // Save as SE(3) we have hat(^) and vee(v)
